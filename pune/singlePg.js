...
//    map1.src = queryParams.mapSrc
   ...
   findLocation(queryParams.location)
}
...
async function findLocation(location) {
    ...
    const coordinates = await findCoordinates(location);
    if (coordinates) {
        setMapCenter(coordinates.lat, coordinates.lon, coordinates.display_name);
    }
}

function initMap() {
    map = L.map('map').setView([20.5937, 78.9629], 18); // Default to India center
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: 'Map data © <a href="https://openstreetmap.org">OpenStreetMap</a> contributors'
    }).addTo(map);
}

function setMapCenter(lat, lon, display_name) {
    if (marker) {
        map.removeLayer(marker);
    }
    map.setView([lat, lon], 15);
    marker = L.marker([lat, lon]).addTo(map);
    marker.bindPopup(`<b>${display_name}</b><br>Lat: ${lat}, Lon: ${lon}`).openPopup();
}
